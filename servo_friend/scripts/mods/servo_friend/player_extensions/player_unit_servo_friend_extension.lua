...
PlayerUnitServoFriendExtension.extensions_settings_changed = function(self)
    if self:servo_friend_alive() then
        local pt = self:pt()
...
PlayerUnitServoFriendExtension.spawn_servo_friend = function(self, dt, t)
    ...
    if self.initialized and self.all_packages_loaded and not self:servo_friend_alive() then
...
PlayerUnitServoFriendExtension.update_movement = function(self, dt, t)
    ...
    if self:servo_friend_alive() then
        ...
        -- Update movement
        local current_position = unit_local_position(self.servo_friend_unit, 1)
        ...
        -- Set new position
        unit_set_local_position(self.servo_friend_unit, 1, new_position - vector3(0, 0, math_abs(self.lean) * 0.0005))
...
PlayerUnitServoFriendExtension.on_settings_changed = function(self, setting_id)
    ...
    if not self:servo_friend_alive() and not self.is_local_unit then
    ...
    if setting_id == "mod_option_appearance" then
        ...
        self:respawn_servo_friend(dt, t)
    end
    -- Relay to extensions
...
-- PlayerUnitServoFriendExtension.load_packages = function(self)
--     for _, package_name in pairs(packages_to_load) do
--         local callback = callback(self, "cb_on_package_loaded", package_name)
--         self.loading_packages[package_name] = self.package_manager:load(package_name, REFERENCE, callback)
--     end
-- end

-- PlayerUnitServoFriendExtension.cb_on_package_loaded = function(self, package_name)
--     mod:print("Package loaded: " .. package_name)
--     self.loaded_packages[package_name] = true
--     if table_size(self.loaded_packages) == table_size(self.loading_packages) then
--         mod:print("All packages loaded")
--         self.all_packages_loaded = true
--     end
-- end

-- PlayerUnitServoFriendExtension.release_packages = function(self)
--     for package_name, package_id in pairs(self.loading_packages) do
...
mod.unit_to_first_person_extension_wrapper = function(self, unit_or_extension)
    if type(unit_or_extension) == "userdata" then
...
mod.initialize_player_unit = function(self, first_person_extension)
    ...
    if first_person_extension and not pt.player_unit_extensions[first_person_extension._unit] then
...
mod.destroy_player_unit = function(self, first_person_extension)
    local pt = mod:pt()
    first_person_extension = self:unit_to_first_person_extension_wrapper(first_person_extension)

    if first_person_extension and pt.player_unit_extensions[first_person_extension._unit] then
        pt.player_unit_extensions[first_person_extension._unit]:destroy()
        script_unit_remove_extension(first_person_extension._unit, "player_unit_servo_friend_system")
        pt.player_unit_extensions[first_person_extension._unit] = nil
    end
end
