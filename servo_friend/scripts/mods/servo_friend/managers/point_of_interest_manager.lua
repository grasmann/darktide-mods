...
ServoFriendPointOfInterestManager.is_valid_point = function(self, object)
    return object and not object.__deleted
end
...
ServoFriendPointOfInterestManager.update = function(self, dt, t)
    if self:is_initialized() then
        ...
        for object, interest_type in pairs(self.points) do
            if not self:is_valid_point(object) then
                stale_objects[#stale_objects + 1] = object
            else
                local closest_extension = nil
                local closest_distance = math_huge

                -- Find closest servo_friend
                for unit, extension in pairs(pt.player_unit_extensions) do
                    if self:extension_valid(extension) and self:is_unit_alive(extension.servo_friend_unit) then
                        ...
                        local point_of_interest_extension = mod:servo_friend_extension(extension.servo_friend_unit,
                            "servo_friend_point_of_interest_system")

                        if self:extension_valid(point_of_interest_extension) then
                            local found_position, target_position = point_of_interest_extension
                                :validate_point_of_interest(
                                    object, interest_type)
        ...
        for i = 1, #stale_objects do
            self:on_servo_friend_point_of_interest_removed(stale_objects[i])
        end
    end
end
...
ServoFriendPointOfInterestManager.on_servo_friend_point_of_interest_created = function(self, object, interest_type)
    if self:is_valid_point(object) then
