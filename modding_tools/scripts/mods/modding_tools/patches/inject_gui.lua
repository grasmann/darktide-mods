...
mod.forward_gui = function(self)
    ...
    if instance then
        if instance._ui_forward_renderer then
            -- Return forward gui
            return instance._ui_forward_renderer.gui,
                instance._ui_forward_renderer.gui_retained
        else
            -- Inject forward gui
            self:inject_forward_gui(instance)
            -- Return forward gui
            if instance._modding_tools_ui_forward_renderer then
                return instance._modding_tools_ui_forward_renderer.gui,
                    instance._modding_tools_ui_forward_renderer.gui_retained
            end
        end
    end
end
...
mod.inject_forward_gui = function(self, instance)
    ...
    if not instance._modding_tools_hooked then
        mod:echo("Injecting forward gui to "..view_name)
        -- Hook on enter
