...
mod:hook_require("scripts/managers/ui/ui_profile_spawner", function(instance)
	...
	instance.is_custoization_tab = function(self)
	...
	-- 	if self:is_custoization_tab() then
...
mod:hook(CLASS.UIProfileSpawner, "wield_slot", function(func, self, slot_id, ...)
	...
	-- 	local profile = self._loading_profile_data.profile
	-- 	local p = self._loading_profile_data.position
	-- 	local position = Vector3(p[1], p[2], p[3])
	-- 	local rotation = QuaternionBox.unbox(self._loading_profile_data.rotation)
	-- 	local state_machine = self._loading_profile_data.state_machine
	-- 	self:spawn_profile(profile, position, rotation, nil, state_machine)
	-- end
	-- self:reset()
	-- Original function
	func(self, slot_id, ...)
...
-- 	-- local physics_world = world_physics_world(self._world)
-- 	-- if physics_world and input_service:get("left_pressed") then
-- 	-- 	local cursor = input_service:get("cursor") or NilCursor
-- 	-- 	local from = camera_screen_to_world(self._camera, vector3(cursor[1], cursor[2], 0), 0)
-- 	-- 	local direction = camera_screen_to_world(self._camera, cursor, 1) - from
-- 	-- 	local to = vector3_normalize(direction)
-- 	-- 	local hit_unit, hit_actor = self:_get_raycast_hit(from, to, physics_world, "filter_player_detection")
-- 	-- 	if hit_unit then
-- 	-- 		return unit_3p == hit_unit
-- 	-- 	end
-- 	-- end
-- 	return func(self, input_service, ...)
-- end)
...
mod:hook(CLASS.UIProfileSpawner, "_get_raycast_hit", function(func, self, from, to, physics_world, collision_filter, ...)
	...
	local result, other = PhysicsWorld.raycast(physics_world, from, to, 30, "all", "collision_filter", collision_filter)
