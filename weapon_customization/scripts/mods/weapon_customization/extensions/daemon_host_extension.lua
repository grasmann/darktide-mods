...
local DaemonHostExtension = class("DaemonHostExtension", "WeaponCustomizationExtension")
...
-- Initialize
DaemonHostExtension.init = function(self, extension_init_context, unit, extension_init_data)
    DaemonHostExtension.super.init(self, extension_init_context, unit, extension_init_data)

    self.initialized = true
end

DaemonHostExtension.delete = function(self)
...
mod.get_flashlight_values = function(self)
    -- local flashlight = self:get_flashlight_unit() or self:get_laser_pointer_unit()
    local flashlight = mod:execute_extension(self.player_unit, "flashlight_system", "flashlight_unit")
    if flashlight and unit_alive(flashlight) then
        -- local flashlight_template = self:get_flashlight_template()
        local light_template = mod:execute_extension(self.player_unit, "flashlight_system", "light_template")
        if light_template then
            local distance = light_template.first_person.falloff.far
