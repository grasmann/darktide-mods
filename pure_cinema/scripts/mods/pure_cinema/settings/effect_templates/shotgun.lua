...
local WWISE_GUN_START = "wwise/events/weapon/play_weapon_shotgun_chaos"
local SHOOT_VFX = "content/fx/particles/weapons/rifles/shotgun/shotgun_rifle_muzzle"
local LINE_EFFECT = LineEffects.renegade_pellet
...
local effect_template = {
	...
	start = function (template_data, template_context)
		...
		template_data.muzzle_time = mod:time()
	...
	update = function (template_data, template_context, dt, t)
        ...
        if not template_data.muzzle_particle_id then
            ...
            local end_position = unit_world_position(weapon_unit, 1) + quaternion_forward(unit_world_rotation(weapon_unit, 1)) * 100
			for i = 1, 5, 1 do
				visual_loadout_extension:_trigger_unit_line_fx(LINE_EFFECT, "slot_ranged_weapon", FX_MUZZLE_SOURCE_NAME, end_position + vector3(math_random(1, 15), math_random(1, 15), math_random(1, 15)))
			end

            template_data.muzzle_particle_id = muzzle_particle_id
            template_data.muzzle_time = t

        elseif template_data.muzzle_particle_id and t - template_data.muzzle_time >= .6 then
