...
local effect_template = {
	...
	start = function (template_data, template_context)
		...
		local source_id = wwise_world_make_manual_source(wwise_world, attachment_unit, fx_muzzle_1_node_index)

		wwise_world_trigger_resource_event(wwise_world, WWISE_GUN_START, source_id)

		template_data.source_id = source_id
		template_data.stop_sound = WWISE_GUN_STOP
		...
		local muzzle_1_particle_id = world_create_particles(world, SHOOT_VFX, position, nil, nil, template_data.particle_group)

		world_link_particles(world, muzzle_1_particle_id, attachment_unit, fx_muzzle_1_node_index, pose, ORPHANED_POLICY)

		template_data.muzzle_1_particle_id = muzzle_1_particle_id

		local muzzle_2_particle_id = world_create_particles(world, SHOOT_VFX, position, nil, nil, template_data.particle_group)

		world_link_particles(world, muzzle_2_particle_id, attachment_unit, fx_muzzle_2_node_index, pose, ORPHANED_POLICY)

		template_data.muzzle_2_particle_id = muzzle_2_particle_id

		unit_animation_event(unit, "offset_rifle_standing_shoot_loop_01")
	...
	stop = function (template_data, template_context)
		...
		if source_id then
			wwise_world_trigger_resource_event(wwise_world, WWISE_GUN_STOP, source_id)
