...
-- #region Performance
    ...
    local camera_set_custom_vertical_fov = Camera.set_custom_vertical_fov
    local unit_set_shader_pass_flag_for_meshes = unit.set_shader_pass_flag_for_meshes
    local script_world_viewport = ScriptWorld.viewport
    local script_viewport_camera = ScriptViewport.camera
    local script_camera_force_update = ScriptCamera.force_update
    local mod_cached_table_contains = mod.cached_table_contains
    local mod_collect_fixes = mod.collect_fixes
    local mod_fetch_attachment = mod.fetch_attachment
    local mod_print = mod.print
--#endregion
...
SightExtension.is_charging = function(self, optional_weapon_action)
    local weapon_action = optional_weapon_action or SightExtension.weapon_action(self)
    return mod_cached_table_contains(mod, AIM_ACTIONS, weapon_action)
end
...
SightExtension.update_zoom = function(self, viewport_name, default_vertical_fov, default_custom_vertical_fov)
	...
	if SightExtension.is_in_first_person_mode(self) and SightExtension.has_offset_fov(self) then
		local viewport = script_world_viewport(self.world, viewport_name)
		local camera = viewport and script_viewport_camera(viewport)
		if camera then
			if self.custom_fov then camera_set_custom_vertical_fov(camera, self.custom_fov) end
			if self.fov then camera_set_vertical_fov(camera, self.fov) end
            script_camera_force_update(self.world, camera)
		end
	end
end
...
SightExtension.update = function(self, dt, t)
    ...
    local is_aiming = SightExtension.is_aiming(self)

    if SightExtension.is_wielded(self) and SightExtension.is_in_first_person_mode(self) and (is_aiming or SightExtension.is_charging(self)) then
        local offset_position = offset.position and vector3_unbox(offset.position) or vector3_zero()
        ...
        if custom_fov and fov and SightExtension.has_default_fov(self) then
            self.custom_fov = math_lerp(self.custom_fov, custom_fov, dt * 20)
            self.fov = math_lerp(self.fov, fov, dt * 20)
        end
    ...
    -- Dof
    if mod_cached_table_contains(mod, DOF_SLOTS, self.wielded_slot) then
        local target_dof = is_aiming and self.dof_aim_target or self.dof_target
        self.dof_near_scale = math_lerp(self.dof_near_scale, target_dof, dt * 10) * (self.dof_strength / 2)
    else
        self.dof_near_scale = math_lerp(self.dof_near_scale, 0, dt * 10)
    end
...
SightExtension.fetch_sight_offset = function(self, item)
    ...
    if self.weapon and self.weapon.attachments then
        self.sight_name = mod_fetch_attachment(mod, self.weapon.attachments, "sight_2") or mod_fetch_attachment(mod, self.weapon.attachments, "sight")

        local sight_offset_fixes = mod_collect_fixes(mod, self.weapon, "sight_offset")
        if sight_offset_fixes then
            for fix, attachment_slot in pairs(sight_offset_fixes) do
                self.sight_fix = fix
            end
        ...
        local sight_unit_fixes = mod_collect_fixes(mod, self.weapon, "sight")
        if sight_unit_fixes then
            for fix, attachment_slot in pairs(sight_unit_fixes) do
                self.sight_default_scale = fix.offset and fix.offset.scale or self.sight_default_scale
            end
        end
            ...
            for i = 1, #attachments_1p do
                ...
                if attachment_unit and unit_alive(attachment_unit) then
                    ...
                    if attachment_slot == "lense_1" then
                        self.lense_1_unit = attachment_unit
                        unit_set_shader_pass_flag_for_meshes(self.lense_1_unit, "one_bit_alpha", true, true)
                        mod_print(mod, "lense 1 unit: "..tostring(self.lense_1_unit))
                    elseif attachment_slot == "lense_2" then
                        self.lense_2_unit = attachment_unit
                        unit_set_shader_pass_flag_for_meshes(self.lense_2_unit, "one_bit_alpha", true, true)
                        mod_print(mod, "lense 2 unit: "..tostring(self.lense_2_unit))
                    elseif attachment_slot == "sight" then
                        self.sight_unit = attachment_unit
                        mod_print(mod, "sight unit: "..tostring(self.sight_unit))
                    end
                end
            end
        end
