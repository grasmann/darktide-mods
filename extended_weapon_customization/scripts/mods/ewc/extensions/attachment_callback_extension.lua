...
AttachmentCallbackExtension.init = function(self, extension_init_context, unit, extension_init_data)
    ...
    self.wielded_slot = extension_init_data.wielded_slot
    self.attachments = {}
    self.attachments_array = {} -- Dense array mirror of self.attachments, used for fast ipairs iteration in hot paths (e.g. on_update)
    self.attachment_slots = {}
    -- Register events
    managers.event:register(self, "ewc_reloaded", "on_mod_reload")
    managers.event:register(self, "ewc_settings_changed", "on_settings_changed")
...
AttachmentCallbackExtension.attachment_callback = function(self, callback_name, ...)
    -- Iterate through attachments (dense array -> ipairs is significantly cheaper than pairs on a hash table,
    -- and this runs every frame via on_update)
    local attachments_array = self.attachments_array
    for i = 1, #attachments_array do
        local attachment_slot_data = attachments_array[i]
        local attachment_data = attachment_slot_data.attachment_data
        -- Check callback
        local callback = attachment_data[callback_name]
        if callback then
            -- Call callback
            callback(self, attachment_slot_data, ...) -- Forward arguments
        end
    end
end
        ...
        if attachment_slots then
            ...
            for attachment_slot, data in pairs(attachment_slots) do
                ...
                if attachment_data and attachment_data[function_name] then
                    -- Get attachment unit
                    local found_attachment_units = attachment_units and mod:find_in_units(attachment_units, attachment_slot)
                    if found_attachment_units then
                        local callback = attachment_data[function_name]
                        for index = 1, #found_attachment_units do
                            local attachment_unit = found_attachment_units[index]

                            if attachment_unit and unit_alive(attachment_unit) then
                                -- Execute
                                callback(world, attachment_unit, attachment_data, ...) -- Forward arguments
                            end
...
AttachmentCallbackExtension.fetch_attachments_with_callbacks = function(self)
    -- Null data
    table_clear(self.attachments)
    table_clear(self.attachment_slots)
    local attachments_array = self.attachments_array
    table_clear(attachments_array)
    local array_index = 0
    -- Iterate through supported slots
    for i = 1, #SUPPORTED_SLOTS do
        local slot_name = SUPPORTED_SLOTS[i]
        -- Check if slot exists
        if self.visual_loadout_extension._equipment[slot_name] then
            -- Get item
            local item = self.visual_loadout_extension:item_from_slot(slot_name)
            ...
            if item and item.attachments then
                ...
                local attachments_3p = attachments_by_unit_3p and attachments_by_unit_3p[unit_3p]
                -- Get attachment slots
                local item_attachment_slots = mod:fetch_attachment_slots(item.attachments)
                self.attachment_slots[item] = item_attachment_slots
                -- Iterate through attachments slots
                for attachment_slot, data in pairs(item_attachment_slots) do
                    -- Get attachment data
                    local attachment_data = mod.settings.attachment_data_by_item_string[data.item]
                    if attachment_data then
                        local has_callbacks = false
                        -- Check for callbacks
                        for j = 1, #SUPPORTED_FUNCTIONS do
                            has_callbacks = attachment_data[SUPPORTED_FUNCTIONS[j]]
                            if has_callbacks then break end
                        end
                        -- Check callbacks
                        if has_callbacks then
                            ...
                            self.attachments[attachment_slot] = attachment_entry
                            array_index = array_index + 1
                            attachments_array[array_index] = attachment_entry
                        end
                    end
                end
            end
