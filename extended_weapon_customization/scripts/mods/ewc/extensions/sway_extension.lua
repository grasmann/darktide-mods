...
-- Vorab berechnete, konstante Vektoren für den Crouch-Effekt.
-- Diese Werte ändern sich nie zur Laufzeit, daher werden sie einmalig
-- angelegt statt jeden Frame in update() neu alloziert zu werden.
-- (Ogryn- und Human-Rotation waren im Original ohnehin identisch: vector3(0, -60, 0))
-- Vector3 darf nicht roh am Skript-Anfang gehalten werden -> über Vector3Box
-- speichern und in update() jeweils mit vector3_unbox wieder auspacken.
local ZERO_VECTOR_BOX = vector3_box(vector3_zero())
local CROUCH_ROTATION_BOX = vector3_box(vector3(0, -60, 0))
local CROUCH_POSITION_OGRYN_BOX = vector3_box(vector3(0, 0, -.35))
local CROUCH_POSITION_HUMAN_BOX = vector3_box(vector3(0, 0, -.15))
...
SwayExtension.update = function(self, dt, t)
    ...
    if self:is_in_first_person_mode() then
        ...
        if first_person_unit and unit_alive(first_person_unit) then
            ...
            -- Cache repeatedly used values/self-field lookups once per frame
            local is_aiming = self:is_aiming()
            local wielded_slot = self.wielded_slot

            -- Update crouch
            if self.crouch and (wielded_slot ~= SLOT_PRIMARY or self.crouch_melee) then

                local apply_crouching = self:is_crouching() and self:weapon_action_allowed() and not is_aiming

                local crouch_rotation = vector3_unbox(ZERO_VECTOR_BOX)
                local crouch_position = vector3_unbox(ZERO_VECTOR_BOX)

                if apply_crouching then
                    crouch_rotation = vector3_unbox(CROUCH_ROTATION_BOX)
                    crouch_position = vector3_unbox(self:is_ogryn() and CROUCH_POSITION_OGRYN_BOX or CROUCH_POSITION_HUMAN_BOX)
                end
            ...
            if self.sway then
                ...
                local min, max = -2.5, 2.5
                local aim_factor = is_aiming and .5 or 1
                local diff = vector3_unbox(self.rotation) - rotation
                local momentum_x = math_clamp(diff[3], min, max) * aim_factor
                local momentum_y = math_clamp(diff[1], min, max) * aim_factor

                local self_momentum_x = self.momentum_x

                if (momentum_x > 0 and momentum_x > self_momentum_x) or (momentum_x < 0 and momentum_x < self_momentum_x) then
                    self_momentum_x = math_lerp(self_momentum_x, momentum_x, dt * 4)
                else
                    self_momentum_x = math_lerp(self_momentum_x, momentum_x, dt * 10)
                end

                self.momentum_x = self_momentum_x

                local self_momentum_y = math_lerp(self.momentum_y, momentum_y, dt * 4)
                self.momentum_y = self_momentum_y

                -- Note: self_momentum_x is always clamped to [-2.5, 2.5] (see math_clamp above),
                -- which is well within (-180, 180]. For any value in that range the original
                -- "% 360 -> shift -> unwrap" normalization is mathematically a no-op, so it
                -- simplifies to a plain negation without changing the result.
                local angle_x = -self_momentum_x * .05
                local momentum_y_scaled = self_momentum_y * .05

                unit_position = unit_position + vector3(angle_x, 0, momentum_y_scaled)
                unit_rotation = quaternion_multiply(unit_rotation, quaternion_from_vector(vector3(momentum_y_scaled, -angle_x * 60, angle_x)))
