...
DamageTypeExtension.fetch_damage_types = function(self)
    ...
    local self_damage_types = self.damage_types
    local self_merged_damage_types = self.merged_damage_types
    local attachment_data_by_item_string = mod.settings.attachment_data_by_item_string
    local mod_damage_types = mod.damage_types
    local visual_loadout_extension = self.visual_loadout_extension

    table_clear(self_damage_types)
    table_clear(self_merged_damage_types)

    for i = 1, #PROCESS_SLOTS do
        local slot_name = PROCESS_SLOTS[i]

        local dt_slot = {}
        local merged_slot = {}
        self_damage_types[slot_name] = dt_slot
        self_merged_damage_types[slot_name] = merged_slot
        ...
        if weapon and weapon.attachments then
            ...
            if attachment_slots then
                ...
                for attachment_slot, data in pairs(attachment_slots) do
                    ...
                    local attachment_data = attachment_data_by_item_string[data.item]

                    if attachment_data and attachment_data.damage_type then

                        local damage_type = mod_damage_types[attachment_data.damage_type]

                        if damage_type then
                            dt_slot[attachment_slot] = damage_type
                        end
                ...
                for j = 1, #PRIORITY_SLOTS do
                    local attachment_slot = PRIORITY_SLOTS[j]
                    local damage_type = dt_slot[attachment_slot]

                    if damage_type then

                        merged_slot = table_merge_recursive_n(nil, merged_slot, damage_type)
                        TEMP_WAS_MERGED[attachment_slot] = true
                ...
                for attachment_slot in pairs(attachment_slots) do

                    if not TEMP_WAS_MERGED[attachment_slot] then
