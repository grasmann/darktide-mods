...
local SLOT_SECONDARY = "slot_secondary"
local PROCESS_SLOTS = {SLOT_PRIMARY, SLOT_SECONDARY}
local PROCESS_SLOTS_COUNT = #PROCESS_SLOTS

-- Precomputed slot_name -> index lookup, so the hot per-frame loop never has
-- to do string comparisons/hashing to figure out which slot is wielded.
local SLOT_INDEX_BY_NAME = {}
for i = 1, PROCESS_SLOTS_COUNT do
    SLOT_INDEX_BY_NAME[PROCESS_SLOTS[i]] = i
end
...
ShieldTransparencyExtension.init = function(self, extension_init_context, unit, extension_init_data)
    ...
    self.block_component = self.unit_data_extension:read_component("block")
    -- Settings
    -- shield_units is now a small numerically indexed array (parallel to
    -- PROCESS_SLOTS) instead of a string-keyed hash table, so the per-frame
    -- update loop can use cheap numeric indexing/iteration instead of pairs().
    self.shield_units = {}
    self.wielded_slot = extension_init_data.wielded_slot
    self.wielded_slot_index = SLOT_INDEX_BY_NAME[self.wielded_slot]
    self.transparency = 0
    -- Register Events
    managers.event:register(self, "ewc_reloaded", "on_mod_reload")
    managers.event:register(self, "ewc_settings_changed", "on_settings_changed")
...
ShieldTransparencyExtension.on_wield = function(self, wielded_slot)
    self.wielded_slot = wielded_slot
    -- Cache the numeric index once here instead of comparing strings every
    -- frame inside update().
    self.wielded_slot_index = SLOT_INDEX_BY_NAME[wielded_slot]
end
...
-- Kept as public API (unchanged signature/behavior) in case other code calls
-- them, but the hot update() loop below no longer goes through these to
-- avoid the extra method-dispatch overhead every frame.
ShieldTransparencyExtension.is_aiming = function(self)
    return self.alternate_fire_component and self.alternate_fire_component.is_active
end
...
ShieldTransparencyExtension.update = function(self, dt, t)
    ...
    -- Localize hot fields once instead of repeated self.* table lookups.
    local alternate_fire_component = self.alternate_fire_component
    local block_component = self.block_component
    local shield_units = self.shield_units
    local wielded_slot_index = self.wielded_slot_index
    local transparency = self.transparency

    local is_aiming = alternate_fire_component and alternate_fire_component.is_active
    local is_blocking = block_component and block_component.is_blocking

    if (is_aiming or is_blocking) and self:is_in_first_person_mode() then
        transparency = math_lerp(transparency, self.shield_transparency, dt * 10)
    else
        transparency = math_lerp(transparency, 0, dt * 10)
    end

    self.transparency = transparency

    -- Numeric loop over a fixed-size array instead of pairs() over a
    -- string-keyed hash table, and integer comparison instead of a string
    -- comparison to find the wielded slot.
    for i = 1, PROCESS_SLOTS_COUNT do
        local shield_unit = shield_units[i]

        if shield_unit and unit_alive(shield_unit) then
            if i == wielded_slot_index then
                unit_set_scalar_for_materials(shield_unit, "inv_jitter_alpha", transparency, true)
            else
                unit_set_scalar_for_materials(shield_unit, "inv_jitter_alpha", 0, true)
            end
        end
...
ShieldTransparencyExtension.fetch_shield_units = function(self)
    ...
    local shield_units = self.shield_units
    local visual_loadout_extension = self.visual_loadout_extension

    table_clear(shield_units)

    for i = 1, PROCESS_SLOTS_COUNT do
        local slot_name = PROCESS_SLOTS[i]

        local weapon = visual_loadout_extension:item_from_slot(slot_name)
        if weapon and weapon.attachments and mod:is_shield(weapon) then
            local shield_name = mod:fetch_attachment(weapon.attachments, "left")
            mod:print("shield name: "..tostring(shield_name))

            local unit_1p, unit_3p, attachments_by_unit_1p, attachments_by_unit_3p = visual_loadout_extension:unit_and_attachments_from_slot(slot_name)
            local attachments_1p = attachments_by_unit_1p and attachments_by_unit_1p[unit_1p]
            if attachments_1p then
                for j = 1, #attachments_1p do
                    local attachment_unit = attachments_1p[j]
                    if attachment_unit and unit_alive(attachment_unit) then
                        -- local attachment_slot_string = unit_get_data(attachment_unit, "attachment_slot_long")
                        -- local attachment_slot_parts = string_split(attachment_slot_string, ".")
                        -- local attachment_slot_parts = mod:cached_split(attachment_slot_string, ".")
                        ...
                        if attachment_slot == "left" then
                            shield_units[i] = attachment_unit
                            unit_set_shader_pass_flag_for_meshes(attachment_unit, "one_bit_alpha", true, true)
                            mod:print("shield unit: "..tostring(attachment_unit))
                        end
                    end
