...
FlashlightExtension.find_in_units = function(self, attachment_units, optional_target_slot, optional_settings_slot, optional_flashlight_name)
    ...
    -- Check
    if attachment_units then
        -- Hoisted out of the loop: independent of the loop variable, was previously
        -- re-looked-up on every single iteration
        local weapon_attachments = mod.settings.attachments[self.weapon.weapon_template]
        -- Iterate through attachments
        for i = 1, #attachment_units do
            -- Get attachment unit
            ...
            if attachment_unit and unit_alive(attachment_unit) then
                ...
                -- local attachment_slot_parts = string_split(attachment_slot_string, ".")
                -- local attachment_slot_parts = mod:cached_split(attachment_slot_string, ".")
                -- local attachment_slot_parts = attachment_slot_parts and attachment_slot_parts[#attachment_slot_parts]
                local attachment_slot = unit_get_data(attachment_unit, "attachment_slot")
                -- Check attachment slot and light in attachment unit
                if attachment_slot == target_slot then
                    -- Get flashlight name
...
FlashlightExtension.update = function(self, dt, t)
    ...
    -- if self:is_wielded() then
        ...
        if on ~= self.old_on then
            ...
            -- Computed once and reused below instead of 3x per call
            local timeout_point = t + self.auto_timeout

            if self.auto_timer > timeout_point then
                self.auto_timer = timeout_point
            end
    ...
    local attachment_data = self.attachment_data
    if self.on and attachment_data then
        if perspective_change and attachment_data.on_perspective_change then
            attachment_data.on_perspective_change(self)
        end
        if attachment_data.on_flashlight_update then
            attachment_data.on_flashlight_update(self, dt, t)
        end
    end
...
FlashlightExtension.fetch_flashlight = function(self, item)
    ...
    if self.weapon and self.weapon.attachments then
        ...
        -- Debug
        for _, unit in ipairs(self.flashlight_unit_1p) do
            mod:print("flashlight unit 1p: "..tostring(unit))
        end
        for _, unit in ipairs(self.flashlight_unit_3p) do
            mod:print("flashlight unit 3p: "..tostring(unit))
        end
        for _, name in ipairs(self.flashlight_name) do
            mod:print("flashlight name: "..tostring(name))
        end
        ...
        -- Hoisted: these tables keep the same identity across the loop below,
        -- so look them up once instead of via self.* on every iteration
        local flashlight_unit_1p = self.flashlight_unit_1p
        local flashlight_unit_3p = self.flashlight_unit_3p
        local flashlight_attachment_data = self.flashlight_attachment_data

        for index, template in ipairs(self.flashlight_template_names) do
            mod:print("flashlight template name: "..tostring(template))
            ...
            if template then
                ...
                -- Get lights
                self.flashlight_1p = self.flashlight_1p or {}
                self.flashlight_1p[index] = unit_light(flashlight_unit_1p[index], 1)
                self.flashlight_3p = self.flashlight_3p or {}
                self.flashlight_3p[index] = unit_light(flashlight_unit_3p[index], 1)

                local flashlight_1p = self.flashlight_1p
                local flashlight_3p = self.flashlight_3p

                for index2, light_1p in ipairs(flashlight_1p) do

                    -- Check lights (reuse the value the iterator already gave us
                    -- instead of re-indexing the same table a second time)
                    local light_3p = flashlight_3p[index2]
                    if light_1p and light_3p then
                        
                        -- Init light
                        self:init_light(light_1p)
                        self:init_light(light_3p)
                        -- Set attachment data
                        self.attachment_data = flashlight_attachment_data[index]
                        -- Set light in profile spawner
                        if self.from_ui_profile_spawner then
                            self:set_light(true)
                        end
...
FlashlightExtension.set_light = function(self, value, optional_no_sound)
    ...
    local on = value or not self.on
    self.on = on

    local attachment_data = self.attachment_data
    if attachment_data then
        if on and attachment_data.on_flashlight_on then
            attachment_data.on_flashlight_on(self)
        elseif not on and attachment_data.on_flashlight_off then
            attachment_data.on_flashlight_off(self)
        end
    end
    ...
    if not self:is_in_hub() then
        ...
        local flashlight_unit_1p = self.flashlight_unit_1p
        local flashlight_1p = self.flashlight_1p
        if flashlight_unit_1p and flashlight_1p then
            for index, flashlight in ipairs(flashlight_1p) do
                self:_set_light(flashlight_unit_1p[index], flashlight, on, optional_no_sound)
            end
        end
        local flashlight_unit_3p = self.flashlight_unit_3p
        local flashlight_3p = self.flashlight_3p
        if flashlight_unit_3p and flashlight_3p then
            for index, flashlight in ipairs(flashlight_3p) do
                self:_set_light(flashlight_unit_3p[index], flashlight, on, optional_no_sound)
            end
        end
...
FlashlightExtension._set_light = function(self, flashlight_unit, flashlight, value, optional_no_sound)
    ...
    if flashlight and flashlight_unit and unit_alive(flashlight_unit) then
        ...
        local on = self.on
        -- Set light
        mod:set_light(flashlight, on)
        -- Play sound
        if not optional_no_sound and self:is_modded() then
            local fx_extension = self.fx_extension
            if fx_extension then
                if on then
                    -- Switch on
                    fx_extension:trigger_wwise_event("wwise/events/player/play_foley_gear_flashlight_on", false, self.unit, 1)
                else
                    -- Switch off
                    fx_extension:trigger_wwise_event("wwise/events/player/play_foley_gear_flashlight_off", false, self.unit, 1)
                end
            end
        end
    end
...
FlashlightExtension.init_light = function(self, light)
    if not self:is_in_hub() and self.flashlight_1p and self.flashlight_template then
        ...
        local flashlight_3p = self.flashlight_3p
        local flashlight_template = self.flashlight_template

        for index, light_1p in ipairs(self.flashlight_1p) do

            local template = flashlight_template[index]
            if template then
                if light_1p then self:_init_light(light_1p, template) end
                local light_3p = flashlight_3p[index]
                if light_3p then self:_init_light(light_3p, template) end
            end
...
mod.set_template_for_light = function(self, light, template)
    if light then
        ...
        light_set_volumetric_intensity(light, template.volumetric_intensity)
        light_set_casts_shadows(light, self.flashlight_shadows)
        local spot_angle = template.spot_angle
        light_set_spot_angle_start(light, spot_angle.min)
        light_set_spot_angle_end(light, spot_angle.max)
        light_set_spot_reflector(light, template.spot_reflector)
        local falloff = template.falloff
        light_set_falloff_start(light, falloff.near)
        light_set_falloff_end(light, falloff.far)
        if template.color_filter then
            local color_filter = vector3_unbox(template.color_filter)
            light_set_color_filter(light, color_filter)
        end
