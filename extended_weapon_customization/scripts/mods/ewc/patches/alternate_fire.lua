...
-- visual_loadout_extension / secondary_item are optional pre-fetched values to avoid a
-- redundant script_unit_extension + item_from_slot lookup when the caller already has
-- them (e.g. the hot per-frame call sites further down).
mod.alternate_fire_find_sight_attachment = function(self, player_unit, visual_loadout_extension, secondary_item)
    ...
    visual_loadout_extension = visual_loadout_extension or script_unit_extension(player_unit, "visual_loadout_system")
    if visual_loadout_extension then
        local item = secondary_item or visual_loadout_extension:item_from_slot("slot_secondary")
        if item then
            local sight_attachment_path = self:fetch_attachment(item.attachments, "sight")
            local sight_attachment = sight_attachment_path and self.settings.attachment_data_by_item_string[sight_attachment_path]
            if sight_attachment then
...
-- visual_loadout_extension / secondary_item are optional pre-fetched values, see
-- alternate_fire_find_sight_attachment above.
mod.alternate_fire_override = function(self, unit, value_name, visual_loadout_extension, secondary_item)
    visual_loadout_extension = visual_loadout_extension or script_unit_extension(unit, "visual_loadout_system")
    if visual_loadout_extension then
        local sight_attachment = mod:alternate_fire_find_sight_attachment(unit, visual_loadout_extension, secondary_item)
        if sight_attachment then
            local alternate_fire_override = sight_attachment and sight_attachment.alternate_fire and alternate_fire_overrides[sight_attachment.alternate_fire]
            if alternate_fire_override and alternate_fire_override[value_name] then
                local item = secondary_item or visual_loadout_extension:item_from_slot("slot_secondary")
...
-- Combined helper: fetches the visual_loadout_system extension a single time and
-- derives both the wielded slot and the wielded item from it, instead of calling
-- alternate_fire_wielded_slot()/alternate_fire_wielded_item() separately (which
-- together would perform redundant script_unit_extension lookups per call).
mod.alternate_fire_context = function(self, unit)
    local visual_loadout_extension = script_unit_extension(unit, "visual_loadout_system")
    if not visual_loadout_extension then
        return nil, "slot_unarmed", nil
    end

    local wielded_slot = (visual_loadout_extension._inventory_component and visual_loadout_extension._inventory_component.wielded_slot) or visual_loadout_extension._wielded_slot or "slot_unarmed"
    local wielded_item = visual_loadout_extension:item_from_slot(wielded_slot)

    return visual_loadout_extension, wielded_slot, wielded_item
end
...
mod:hook_require("scripts/utilities/look_delta", function(instance)
    ...
    instance.look_delta_template = function (weapon_action_component, alternate_fire_component, unit)
        ...
        -- ##### Sight extension ######################################################################################
        local visual_loadout_extension, wielded_slot, item = mod:alternate_fire_context(unit)
        local gear_id = mod:gear_id(item)
        local alternate_fire_list = mod:get("alternate_fire")
        local use_alternate_fire = alternate_fire_list and alternate_fire_list[gear_id]
        if use_alternate_fire == nil then use_alternate_fire = true end

        if wielded_slot == SLOT_SECONDARY and use_alternate_fire then
            
            local override = mod:alternate_fire_override(unit, "look_delta_template", visual_loadout_extension, item)
            if override then return LookDeltaTemplates[override] end
...
mod:hook_require("scripts/utilities/alternate_fire", function(instance)
    ...
    instance.camera_variables = function (weapon_template, camera_follow_unit)
        ...
        -- ##### Sight extension ######################################################################################
        if camera_variable_unit then
            local visual_loadout_extension, wielded_slot, item = mod:alternate_fire_context(camera_variable_unit)
            local gear_id = mod:gear_id(item)
            local alternate_fire_list = mod:get("alternate_fire")
            local use_alternate_fire = alternate_fire_list and alternate_fire_list[gear_id]
            if use_alternate_fire == nil then use_alternate_fire = true end
            ...
            if wielded_slot == SLOT_SECONDARY and use_alternate_fire then
                local override = mod:alternate_fire_override(camera_variable_unit, "camera", visual_loadout_extension, item)
                if override == false then return nil, nil, nil
                elseif override then camera_settings = override end
            end
        end
    ...
    instance.start = function (alternate_fire_component, weapon_tweak_templates_component, spread_control_component, sway_control_component, sway_component, movement_state_component, locomotion_component, inair_state_component, peeking_component, first_person_extension, animation_extension, weapon_extension, weapon_template, player_unit, t)
        ...
        -- ##### Sight extension ######################################################################################
        local visual_loadout_extension, wielded_slot, item = mod:alternate_fire_context(player_unit)
        local gear_id = mod:gear_id(item)
        local alternate_fire_list = mod:get("alternate_fire")
        local use_alternate_fire = alternate_fire_list and alternate_fire_list[gear_id]
        if use_alternate_fire == nil then use_alternate_fire = true end
        ...
        if wielded_slot == SLOT_SECONDARY and use_alternate_fire then
            ...
            local override = mod:alternate_fire_override(player_unit, "aim_animation", visual_loadout_extension, item)
            if override then start_anim_event = override end
            local override_3p = mod:alternate_fire_override(player_unit, "aim_animation_3p", visual_loadout_extension, item)
            if override_3p then start_anim_event_3p = override_3p end
    ...
    instance.stop = function (alternate_fire_component, peeking_component, first_person_extension, weapon_tweak_templates_component, animation_extension, weapon_template, player_unit, from_action_input)
        ...
        if wielded_slot == SLOT_SECONDARY and use_alternate_fire then
            ...
            local override = mod:alternate_fire_override(player_unit, "unaim_animation", visual_loadout_extension, item)
            if override then stop_anim_event = override end
            local override_3p = mod:alternate_fire_override(player_unit, "unaim_animation_3p", visual_loadout_extension, item)
            if override_3p then stop_anim_event_3p = override_3p end
