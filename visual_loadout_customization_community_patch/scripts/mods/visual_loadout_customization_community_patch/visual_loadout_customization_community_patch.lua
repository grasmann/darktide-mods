...
mod:hook_require("scripts/extension_systems/visual_loadout/utilities/visual_loadout_customization", function(VisualLoadoutCustomization)
    ...
    VisualLoadoutCustomization.spawn_item = function (item_data, attach_settings, parent_unit, optional_map_attachment_name_to_unit, optional_extract_attachment_units_bind_poses, optional_extract_item_names, optional_mission_template, optional_equipment)
        local weapon_skin = VisualLoadoutCustomization._validate_item_name(item_data.slot_weapon_skin)
    ...
    VisualLoadoutCustomization.spawn_item_attachments = function (item_data, override_lookup, attach_settings, item_unit, optional_map_attachment_name_to_unit, optional_extract_attachment_units_bind_poses, optional_extract_item_names, optional_mission_template, optional_equipment)
        ...
        if attachments then
            ...
            VisualLoadoutCustomization._attach_hierarchy(attachment_slot_data, override_lookup, attach_settings, item_unit, attachment_name, extract_data, optional_map_attachment_name_to_unit, optional_extract_attachment_units_bind_poses, optional_extract_item_names, optional_mission_template, optional_equipment)
            end
        end
    ...
    VisualLoadoutCustomization.attach_hierarchy = function (attachment_slot_data, override_lookup, settings, parent_unit, parent_item_name, attachment_name, optional_map_attachment_name_to_unit, optional_extract_attachment_units_bind_poses, optional_extract_item_names, optional_mission_template)
        ...
        VisualLoadoutCustomization._attach_hierarchy(attachment_slot_data, override_lookup, settings, parent_unit, attachment_name, extract_data, optional_map_attachment_name_to_unit, optional_extract_attachment_units_bind_poses, optional_extract_item_names, optional_mission_template)
    ...
    VisualLoadoutCustomization.generate_attachment_overrides_lookup = function (item_data, override_item_data)
        ...
        for attachment_name, attachment_slot_data in pairs(attachments) do
            ...
            VisualLoadoutCustomization._generate_attachment_overrides_recursive(attachment_slot_data, override_slot_data, override_lookup, item_data)
        end
    ...
    VisualLoadoutCustomization._generate_attachment_overrides_recursive = function (attachment_slot_data, override_slot_data, override_lookup, item)
        if not override_slot_data then
            return
        end

        local override_item_name = VisualLoadoutCustomization._validate_item_name(override_slot_data.item)
        ...
        for name, data in pairs(children) do
            ...
            VisualLoadoutCustomization._generate_attachment_overrides_recursive(data, override_data, override_lookup, item)
        end
    end
    ...
    -- ##### Changed from local function ##############################################################################
    VisualLoadoutCustomization._validate_item_name = function (item)
        if item == "" then
            return nil
        end
    ...
    -- ##### Changed from local function ##############################################################################
    VisualLoadoutCustomization._real_item = function (attach_settings, item, item_name)
        if not item then
            return nil, nil
        end
    ...
    VisualLoadoutCustomization._attach_hierarchy = function (attachment_slot_data, override_lookup, attach_settings, parent_unit, attachment_name, extract_data, optional_map_attachment_name_to_unit, optional_extract_attachment_units_bind_poses, optional_extract_item_names, optional_mission_template, optional_equipment)
        ...
        item, item_name = VisualLoadoutCustomization._real_item(attach_settings, item, item_name)
    ...
    -- ##### Changed from local function ##############################################################################
    VisualLoadoutCustomization._apply_material_override = function (unit, material_override_data, in_editor)
