...
local FIRE_PARTICLE_NAME = "content/fx/particles/player_buffs/buff_fire_trail_01"
...
-- }
local context = {
    preview_fire_blade_particle_id = {},
...
local function despwan_preview_fire_blade(world, attachment_unit, attachment_data)
        
    if context.preview_fire_blade_particle_id[attachment_unit] then

        if world_are_particles_playing(world, context.preview_fire_blade_particle_id[attachment_unit]) then
            world_stop_spawning_particles(world, context.preview_fire_blade_particle_id[attachment_unit])
        end

        world_destroy_particles(world, context.preview_fire_blade_particle_id[attachment_unit])

        context.preview_fire_blade_particle_id[attachment_unit] = nil
...
local function spawn_preview_fire_blade(world, attachment_unit, attachment_data)

    -- despwan_preview_lasers(world, attachment_unit, attachment_data)

    local fire_node = attachment_data.fire_node or 1

    local blade_rotation = unit_world_rotation(attachment_unit, fire_node)
    local blade_position = unit_world_position(attachment_unit, fire_node)

    local particle_id = world_create_particles(world, FIRE_PARTICLE_NAME, blade_position, blade_rotation)
    ...
    world_move_particles(world, particle_id, blade_position, blade_rotation)
...
local function spawn_fire_blade(flashlight_extension)
...
local function update_fire_blade(flashlight_extension, dt, t)
...
local function despawn_fire_blade(flashlight_extension)
...
local function update_fire_blade_visibility(flashlight_extension, wielded_slot)
    local player_visibility = script_unit_has_extension(flashlight_extension.unit, "player_visibility_system")
    local player_invisible = player_visibility and not player_visibility:visible()
    local inventory_view = mod:get_view("inventory_view")
    if wielded_slot ~= SLOT_PRIMARY or player_invisible or inventory_view then
        despawn_fire_blade(flashlight_extension)
    elseif flashlight_extension.on then
        spawn_fire_blade(flashlight_extension)
    end
end

local function respawn_fire_blade(flashlight_extension)
    despawn_fire_blade(flashlight_extension)
    update_fire_blade_visibility(flashlight_extension, flashlight_extension.wielded_slot)
end

return {
    update_fire_blade_visibility = update_fire_blade_visibility,
    despwan_preview_fire_blade = despwan_preview_fire_blade,
    spawn_preview_fire_blade = spawn_preview_fire_blade,
    despawn_fire_blade = despawn_fire_blade,
    respawn_fire_blade = respawn_fire_blade,
    update_fire_blade = update_fire_blade,
    spawn_fire_blade = spawn_fire_blade,
}