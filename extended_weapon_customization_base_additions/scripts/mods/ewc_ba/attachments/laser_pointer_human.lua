...
local context = {
    preview_laser_particle_id = nil,
    preview_laser_w_dot_id = nil
}

local function despwan_preview_lasers(world, attachment_unit, attachment_data)
        
    if context.preview_laser_particle_id then

        if world_are_particles_playing(world, context.preview_laser_particle_id) then
            world_stop_spawning_particles(world, context.preview_laser_particle_id)
        end

        world_destroy_particles(world, context.preview_laser_particle_id)

        context.preview_laser_particle_id = nil

    end

    if context.preview_laser_w_dot_id then

        if world_are_particles_playing(world, context.preview_laser_w_dot_id) then
            world_stop_spawning_particles(world, context.preview_laser_w_dot_id)
        end

        world_destroy_particles(world, context.preview_laser_w_dot_id)

        context.preview_laser_w_dot_id = nil
...
local function spawn_preview_laser(world, attachment_unit, attachment_data)
    ...
    context.preview_laser_particle_id = particle_id
    ...
    context.preview_laser_w_dot_id = laser_w_dot_id
    ...
    if flashlight_extension.laser_particle_id and flashlight_extension.laser_variable_index then
        ...
        if flashlight_unit or not unit_alive(flashlight_unit) then
            ...
            if sway_extension then
                ...
                aim_position = aim_position - crouch_position --matrix4x4_transform(mat, crouch_position)
            end
            ...
            if sight_extension then
                ...
                aim_position = aim_position - sight_position --matrix4x4_transform(mat, sight_position)
            end
...
return {
    laser_pointer_01 = {
        ...
        ui_item_init = function(world, attachment_unit, attachment_data)
            spawn_preview_laser(world, attachment_unit, attachment_data)
        end,
        ...
        on_perspective_change = function(flashlight_extension)
            respawn_laser_pointer(flashlight_extension)
        end,
        on_flashlight_update = function(flashlight_extension, dt, t)
            update_laser_pointer(flashlight_extension, dt, t)
        end,
        on_update_item_visibility = function(flashlight_extension, wielded_slot)
            update_laser_pointer_visibility(flashlight_extension, wielded_slot)
        end,
